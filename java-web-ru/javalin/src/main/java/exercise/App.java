...
public final class App {
    ...
    private static void addRoutes(Javalin app) {
        ...
        app.routes(() -> {
            // ArticleController тот что будет обрабатывать запрос
            path("articles", () -> {

                // GET /articles — список всех статей - listArticles
                get(ArticleController.listArticles);

                // GET /articles/new — вывод формы создания новой статьи - newArticle
                get("new", ArticleController.newArticle);

                // GET /articles/{id} — просмотр конкретной статьи - showArticle
                path("{id}", () -> get(ArticleController.showArticle));

                // POST /articles — создание новой статьи - createArticle
                post(ArticleController.createArticle);

                // GET /articles/{id}/edit — вывод формы редактирования статьи - editArticle
                path("{id}/edit", () -> get(ArticleController.editArticle));

                // POST /articles/{id}/edit — обновление данных статьи - updateArticle
                path("{id}/edit", () -> post(ArticleController.updateArticle));

                // GET /articles/{id}/delete — вывод страницы с подтверждением удаления статьи - deleteArticle
                path("{id}/delete", () -> get(ArticleController.deleteArticle));

                // POST /articles/{id}/edit — обновление данных статьи - updateArticle
                path("{id}/delete", () -> post(ArticleController.destroyArticle));
            });
        });
        // END
    }
